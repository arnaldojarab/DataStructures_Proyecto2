...
class JobLogic:
    ...
    def reset(self) -> None:
        ...
        self._enemy_dropoff_markers.clear()
        self._job_offer_elapsed = 3.0
        self._game_elapsed = 0.0
        self.reputation = 70
        self.enemyReputation = 70
    ...
    def update(self, dt: float, player_x: float, player_y: float, enemy_x: float, enemy_y: float) -> None:
        ...
        while self._job_offer_elapsed >= self._offer_interval:
            if (len(self._pickup_markers) < self.max_active_offers 
                and len(self._dropoff_markers) < self.max_active_offers 
                and len(self._enemy_dropoff_markers) < self.max_active_offers):
                self._launch_a_job()
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        # Enemy Dropoffs (solamente el current)
        currentEnemyJob = self.orders.getCurrentEnemyJob()
        if currentEnemyJob:
            m = next((d for d in self._enemy_dropoff_markers if d.job_id == currentEnemyJob.id), None)
            rect = enemy_dropoff_icon.get_rect(center=(m.px, m.py))
    ...
    def getInventory(self) -> List[Dict[str, Any]]:
        ...
        for jid in self.orders.inventory:
            ...
            out.append(job)
        return out
    def getEnemyInventory(self) -> List[Dict[str, Any]]:
        """Devuelve la lista de objetos Job actualmente en inventario del enemigo."""
        out: List = []
        for jid in self.orders.enemyInventory:
            job: Job = self.jobs.get(jid)
            out.append(job)
        return out
    ...
    def getEnemyInventoryIDs(self) -> List[str]:
        """Devuelve la lista de IDs de objetos Job actualmente en inventario del enemigo."""
        return self.orders.enemyInventory
    ...
    def getHistory(self) -> List[Dict[str, Any]]:
        ...
        return out
    def getEnemyHistory(self) -> List[Dict[str, Any]]:
        """
        Lista de dicts con información histórica del enemigo.
        Cada item: {"job": <Job as dict>, "accepted": bool, "onTime": bool}
        """
        out: List[Dict[str, Any]] = []
        for h in self.orders.enemyHistory:
            job: Job = self.jobs.get(h.job_id)
            out.append({
                "job": job,
                "accepted": h.accepted,
    ...
    def getEnemyHistoryIDs(self) -> List[HistoryEntry]:
        """
        Lista de dicts con información histórica del enemigo.
        Cada item: {"job_id": str, "accepted": bool, "onTime": bool}
        """
        return self.orders.enemyHistory
    
    def getReputation(self) -> int:
        return self.reputation
    
    def getEnemyReputation(self) -> int:
        return self.enemyReputation
    ...
    def getEnemyWeight(self) -> float:
        total = 0.0
        for h in self.orders.enemyInventory:
                job = self.jobs.get(h)
                total += job.weight
        return total
    ...
    def setCurrentEnemyJob(self, job_id: str) -> None:
        self.orders.set_current_enemy_job(job_id)
    # =================== Lógica interna ===================
        ...
        for idx, m in enumerate(self._pickup_markers):
            if self._game_elapsed >= m.expires_at:
                ...
                if self.reputation < 0:
                    self.reputation = 0
                if self.enemyReputation < 0:
                    self.enemyReputation = 0
    ...
    def _check_proximity(self, player_x: float, player_y: float, enemy_x: float, enemy_y: float) -> None:
        ...
        for idx, m in enumerate(self._pickup_markers):
            ...
            if enemyDist <= self._PICKUP_RADIUS_TILES:
                job = self.jobs.get(m.job_id)
                # Aceptar en inventario del enemigo
                if(self.getEnemyWeight() < 5):
                    self.orders.accept_enemy_job(job.id)
                    print(f"Pedido del enemigo aceptado (agregado al inventario), id: {job.id}")
                    # Crear dropoff marker con due_at relativo
                    dx, dy = job.dropoff
                    qx, qy = self._grid_center_to_px(dx, dy)
                    due_at = self._game_elapsed + self._DROPOFF_LATE_AFTER
                    self._enemy_dropoff_markers.append(DropoffMarker(qx, qy, job.id, due_at))
                    to_remove_pickups.append(idx)
                else:
        ...
        # Enemy Dropoffs:
        currentEnemyJob = self.orders.getCurrentEnemyJob()
        idx, m = next(((i, d) for i, d in enumerate(self._enemy_dropoff_markers) if d.job_id == (currentEnemyJob.id if currentEnemyJob else None)), (None, None))
        if idx is not None and m is not None:
          to_remove_enemy_dropoffs: List[int] = []
          mgx = int(m.px // ts)
          mgy = int(m.py // ts)
          enemyDist = abs(mgx - egx) + abs(mgy - egy)
          if enemyDist <= self._DROPOFF_RADIUS_TILES:
              on_time = self._game_elapsed <= m.due_at
              self.orders.mark_enemy_delivered(m.job_id, delivered_on_time=on_time)
              print(f"Pedido del enemigo entregado (removido del inventario y agregado al historial), id: {m.job_id}, onTime={on_time}")
              if on_time:
                  self.enemyReputation += 10  # recompensa por entrega a tiempo
                  if self.enemyReputation > 100:
                      self.enemyReputation = 100
              else:
                  self.enemyReputation -= 10   # penalización por entrega tarde
                  if self.enemyReputation < 0:
                      self.enemyReputation = 0
              to_remove_enemy_dropoffs.append(idx)

          for i in reversed(to_remove_enemy_dropoffs):
              self._enemy_dropoff_markers.pop(i)

    def getRepSpeed(self):
        if self.reputation >= 90:
            return 1.03
        return 1.0
    def getEnemyRepSpeed(self):
        if self.enemyReputation >= 90:
            return 1.03
        return 1.0
    ...
    def save_state(self) -> dict:
        ...
        orders_state = {
            ...
            "enemyInventory": list(self.orders.enemyInventory),  # [job_id]
            "history": [
            ...
            "currentEnemyJob_id": self.orders.currentEnemyJob_id,
        ...
        markers_state = {
            ...
            "enemy_dropoffs": [asdict(m) for m in self._enemy_dropoff_markers],
        }
        ...
        logic_state = {
            ...
            "enemyReputation": self.enemyReputation,
        }
        return {
            "jobs": jobs_list,
            "orders": orders_state,
    ...
    def load_state(self, state: dict) -> bool:
        ...
        try:
            ...
            self.orders.enemyInventory = list(orders_state.get("enemyInventory", []))
            # Historial
            ...
            self.orders.enemyHistory.clear()
            for h in orders_state.get("enemyHistory", []):
                self.orders.enemyHistory.append(
                    HistoryEntry(
                        job_id=h["job_id"],
                        accepted=bool(h["accepted"]),
                        onTime=bool(h["onTime"]),
            ...
            self.orders.currentEnemyJob_id = orders_state.get("currentEnemyJob_id", None)
            # Colas
            ...
            self._enemy_dropoff_markers.clear()
            for m in state.get("markers", {}).get("enemy_dropoffs", []):  
                self._enemy_dropoff_markers.append(
                    DropoffMarker(
                        px=int(m["px"]),
                        py=int(m["py"]),
                        job_id=m["job_id"],
            ...
            self.enemyReputation = int(logic_state.get("enemyReputation", self.enemyReputation))
